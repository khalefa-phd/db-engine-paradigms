...
TEST(TPCH, q1) {
  ...
  std::map<tuple<Char<1>, Char<1>>,
           tuple<Numeric<12, 2>, Numeric<12, 2>, Numeric<12, 4>, Numeric<12, 6>,
                 int64_t>>
      expected = {
...
TEST(TPCH, q9) {
  ...
  auto checkResult = [&](BlockRelation* result) {
    ...
    for (auto& block : *result) {
      ...
      auto sum_profit =
          reinterpret_cast<types::Numeric<12, 4>*>(block.data(sum_profitAttr));
      for (size_t i = 0; i < elementsInBlock; ++i) {
...
TEST(TPCH, q18) {
  ...
  auto checkResult = [&](BlockRelation* result) {
    ...
    for (auto& block : *result) {
      ...
      auto totalprice =
          reinterpret_cast<types::Numeric<12, 2>*>(block.data(totalpriceAttr));
      auto sum = reinterpret_cast<types::Numeric<12, 2>*>(block.data(sumAttr));
      for (size_t i = 0; i < elementsInBlock; ++i) {
        if (expected.find(make_tuple(name[i], custkey[i], orderkey[i],
