      ...
      if (c_mktsegment[i] == c3) {
         ...
         for (size_t j = b; j < e; j++) {
            ...
            if (o_orderdate[order_id] < c1) {
               ...
               entries.emplace_back(make_tuple(l_orderkey[order_id],
                                               o_orderdate[order_id],
...
#ifdef VERBOSE1
   cout << "Customer count " << customer_count << endl;
   cout << "Order count " << order_count << endl;
   ...
   auto found1 = tbb::parallel_reduce(
       ...
       [&](const tbb::blocked_range<size_t>& r, const size_t& f) {
          ...
          for (size_t i = r.begin(), end = r.end(); i != end; ++i) {
             if (c_mktsegment[i] == c3) {
                // cout << "Customer " << c_custkey[i] << endl;
